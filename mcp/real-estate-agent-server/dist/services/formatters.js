 ...
 */
export function truncateResponse(response, additionalInfo) {
...
// Contact Formatters
// ============================================
export function formatContact(contact, options) {
...
export function formatContactList(contacts, options) {
    ...
    for (const contact of contacts) {
        const typeEmoji = getContactTypeEmoji(contact.type);
        const statusEmoji = getContactStatusEmoji(contact.status);
        lines.push(`| ${typeEmoji} ${contact.type} | ${contact.first_name} ${contact.last_name} | ${statusEmoji} ${contact.status} | ${contact.email || "-"} | ${contact.phone || "-"} |`);
    }
    return lines.join("\n");
}
function getContactTypeEmoji(type) {
    const emojis = {
        lead: "🔵",
        buyer: "🟢",
        seller: "🟡",
...
function getContactStatusEmoji(status) {
    const emojis = {
        new: "🆕",
        contacted: "📞",
        qualified: "✅",
...
// Property Formatters
// ============================================
export function formatProperty(property, options) {
    const detailed = typeof options === "boolean" ? options : options?.detailed ?? false;
    ...
    if (chars.surface)
        lines.push(`- **Surface**: ${chars.surface} m²`);
    if (chars.rooms)
        lines.push(`- **Rooms**: ${chars.rooms}`);
    if (chars.bedrooms)
        lines.push(`- **Bedrooms**: ${chars.bedrooms}`);
    if (chars.bathrooms)
        lines.push(`- **Bathrooms**: ${chars.bathrooms}`);
    if (chars.year_built)
        lines.push(`- **Year Built**: ${chars.year_built}`);
    if (property.tags.length > 0)
        lines.push(`- **Tags**: ${property.tags.join(", ")}`);
    if (property.images.length > 0)
        lines.push(`- **Images**: ${property.images.length} photos`);
    lines.push(`- **ID**: \`${property.id}\``);
    return lines.join("\n");
}
export function formatPropertyList(properties, options) {
    ...
    for (const property of properties) {
        ...
        lines.push(`| ${property.reference} | ${typeEmoji} ${property.type} | ${address} | ${formatPrice(property.price)} | ${statusEmoji} ${property.status} |`);
    }
    return lines.join("\n");
}
...
function getPropertyStatusEmoji(status) {
    const emojis = {
        draft: "📝",
        published: "🟢",
        under_offer: "🟡",
...
// Visit Formatters
// ============================================
export function formatVisit(visit, options) {
...
export function formatVisitTable(visits) {
    if (visits.length === 0) {
    ...
    for (const { visit, property, contact } of visits) {
        ...
        lines.push(`| ${visit.start_time} | ${propertyRef} | ${contactName} | ${statusEmoji} ${confirmEmoji} |`);
    }
    return lines.join("\n");
}
function getVisitStatusEmoji(status) {
    const emojis = {
        scheduled: "📅",
        confirmed: "✅",
        completed: "🏁",
...
function getTaskPriorityEmoji(priority) {
    const emojis = {
        low: "🟢",
        medium: "🟡",
        high: "🟠",
 ...
 */
export function formatPaginatedResponse(response, itemFormatter) {
    const lines = [];
...
// ============================================
export function formatVisitList(visits, pagination) {
    if (visits.length === 0) {
    ...
    for (const { visit, property, contact } of visits) {
        ...
        const propertyRef = property?.reference || visit.property_id;
        const contactName = contact ? `${contact.first_name} ${contact.last_name}` : visit.contact_id;
        lines.push(`| ${visit.date} | ${visit.start_time} | ${propertyRef} | ${contactName} | ${statusEmoji} ${confirmEmoji} |`);
    }
    return lines.join("\n");
}
// ============================================
// Contract Formatters
// ============================================
export function formatContract(contract, options) {
    ...
    if (contract.signed_at)
        lines.push(`- **Signed**: ${contract.signed_at.split("T")[0]}`);
    if (contract.expires_at)
        lines.push(`- **Expires**: ${contract.expires_at.split("T")[0]}`);
    if (contract.file_url)
        lines.push(`- **File**: [View Document](${contract.file_url})`);
    lines.push(`- **ID**: \`${contract.id}\``);
    return lines.join("\n");
}
export function formatContractList(contracts, pagination) {
    ...
    for (const { contract, property, contact } of contracts) {
        ...
        lines.push(`| ${typeEmoji} ${contract.type} | ${propertyRef} | ${contactName} | ${statusEmoji} ${contract.status} |`);
    }
    return lines.join("\n");
}
