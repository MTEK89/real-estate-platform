 ...
 */
import { z } from "zod";
import { VisitStatusSchema, AgencyIdSchema, } from "../../schemas/common.js";
import { getSupabaseClient, getById, insertRecord, updateRecord, } from "../../services/supabase.js";
 ...
 */
export function registerVisitTools(server) {
    // List visits
    server.tool("re_list_visits", `List scheduled visits with optional filters.
...
- "All visits for property APT-001" → property_query: "APT-001"`, {
        ...
        try {
            ...
            return createSuccessResponse(formatVisitList(visits, {
                total: count || 0,
                offset: args.offset,
                limit: args.limit,
            }));
    ...
    // Get visit details
    server.tool("re_get_visit", `Get details of a specific visit.`, {
    ...
    // Create visit
    server.tool("re_create_visit", `Schedule a new property visit/viewing.
...
- "Book a viewing with Marie Dupont for the house on Grand Rue next Monday at 10am"`, {
        ...
        try {
            ...
            const { data, error } = await insertRecord("visits", visitData);
            if (error) {
                return createErrorResponse(error);
            }
            if (!data) {
            ...
            return createSuccessResponse(`# Visit Scheduled!\n\n` +
                `**Date**: ${dateStr}\n` +
    ...
    // Update visit status
    server.tool("re_update_visit_status", `Update a visit's status.
...
- "Cancel tomorrow's visit"`, {
        ...
        try {
            ...
            const { data, error } = await updateRecord("visits", args.id, args.agency_id, updates);
            if (error) {
                return createErrorResponse(error);
            }
            if (!data) {
            ...
            return createSuccessResponse(`# Visit Updated!\n\n` +
                `**Status**: ${data.status}\n` +
    ...
    // Add visit feedback
    server.tool("re_add_visit_feedback", `Record feedback after a property visit.
...
- "John wasn't interested, too far from work"`, {
        ...
        try {
            ...
            const { data, error } = await updateRecord("visits", args.id, args.agency_id, {
                feedback,
                status: "completed",
            });
            if (error) {
            ...
            return createSuccessResponse(`# Feedback Recorded!\n\n` +
    ...
    // Reschedule visit
    server.tool("re_reschedule_visit", `Reschedule an existing visit to a new date/time.
...
- "Reschedule to December 20th"`, {
        ...
        try {
            ...
            const { data: existing, error: fetchError } = await getById("visits", args.id, args.agency_id);
            ...
            return createSuccessResponse(`# Visit Rescheduled!\n\n` +
                `**New Date**: ${dateStr}\n` +
    ...
    // Cancel visit
    server.tool("re_cancel_visit", `Cancel a scheduled visit.`, {
        id: z.string().describe("Visit ID"),
        ...
        try {
            ...
            return createSuccessResponse(`# Visit Cancelled\n\n` +
