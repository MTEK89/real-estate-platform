 ...
 */
import { z } from "zod";
import { ContactTypeSchema, ContactStatusSchema, AgencyIdSchema, } from "../../schemas/common.js";
import { queryList, getById, insertRecord, updateRecord, deleteRecord, } from "../../services/supabase.js";
...
export function registerContactTools(server) {
    // List contacts
    server.tool("re_list_contacts", `List contacts with filtering and search.
...
- "Find contacts tagged with 'investor'"`, {
        ...
        try {
            const { data, count, error } = await queryList({
                table: "contacts",
            ...
            return createSuccessResponse(formatContactList(data, {
                total: count,
                offset: args.offset,
                limit: args.limit,
            }));
    ...
    // Get single contact
    server.tool("re_get_contact", `Get detailed information about a specific contact.
...
- "Show me contact abc-123"`, {
    ...
    server.tool("re_upsert_contact", `Create a new contact or update existing one.
...
- "Update contact status to qualified"`, {
        ...
        try {
            ...
            if (args.id) {
                // Update existing
                const { data, error } = await updateRecord("contacts", args.id, args.agency_id, contactData);
                if (error) {
                    return createErrorResponse(error);
                }
                if (!data) {
                ...
                // Create new
                const { data, error } = await insertRecord("contacts", contactData);
                if (error) {
                    return createErrorResponse(error);
                }
                if (!data) {
            ...
            return createSuccessResponse(`# Contact ${action.charAt(0).toUpperCase() + action.slice(1)}!\n\n` +
    ...
    // Delete contact
    server.tool("re_delete_contact", `Delete a contact.
...
- "Remove this contact"`, {
        ...
        try {
            ...
            const { data: contact } = await getById("contacts", args.id, args.agency_id);
    ...
    server.tool("re_update_contact_status", `Update a contact's status in the pipeline.
...
- "Close this lead as converted"`, {
        ...
        try {
            ...
            const { data, error } = await updateRecord("contacts", result.contact.id, args.agency_id, updates);
            if (error) {
                return createErrorResponse(error);
            }
            if (!data) {
    ...
    // Tag contact
    server.tool("re_tag_contact", `Add or remove tags from a contact.
...
- "Remove 'inactive' tag"`, {
        ...
        try {
            ...
            const { data, error } = await updateRecord("contacts", result.contact.id, args.agency_id, { tags });
            if (error) {
                return createErrorResponse(error);
            }
            return createSuccessResponse(`# Tags Updated!\n\n` +
