 ...
 */
import { z } from "zod";
import { TaskPrioritySchema, TaskStatusSchema, AgencyIdSchema, } from "../../schemas/common.js";
...
export function registerTaskTools(server) {
    // List tasks
    server.tool("re_list_tasks", `List tasks with optional filters.
...
- "Tasks for property APT-001" → related_type: "property", related_id: "[property_id]"`, {
        ...
        try {
            ...
            const tasks = data;
            return createSuccessResponse(formatTaskList(tasks, {
                total: count || 0,
                offset: args.offset,
                limit: args.limit,
            }));
    ...
    // Get task details
    server.tool("re_get_task", `Get details of a specific task.`, {
        id: z.string().describe("Task ID"),
        agency_id: AgencyIdSchema,
    }, async (args) => {
        try {
            const { data, error } = await getById("tasks", args.id, args.agency_id);
            if (error || !data) {
    ...
    // Create task
    server.tool("re_create_task", `Create a new task.
...
- "Follow up with property owner next week" → title: "Follow up with property owner", due_date: "next week"`, {
        ...
        try {
            ...
            const { data, error } = await insertRecord("tasks", taskData);
            if (error) {
                return createErrorResponse(error);
            }
            if (!data) {
            ...
            return createSuccessResponse(`# Task Created!\n\n` +
    ...
    // Update task
    server.tool("re_update_task", `Update an existing task.`, {
        id: z.string().describe("Task ID"),
        ...
        try {
            const { data: existing, error: fetchError } = await getById("tasks", args.id, args.agency_id);
            ...
            const { data, error } = await updateRecord("tasks", args.id, args.agency_id, updateData);
            if (error) {
                return createErrorResponse(error);
            }
            if (!data) {
            ...
            return createSuccessResponse(`# Task Updated!\n\n` + formatTask(data, { detailed: true }));
        }
        catch (error) {
            return createErrorResponse(error);
        }
    ...
    // Complete task
    server.tool("re_complete_task", `Mark a task as completed.
...
- "Complete task [id]"`, {
        ...
        try {
            ...
            const description = args.notes
                ...
                : existing.description;
            const { data, error } = await updateRecord("tasks", args.id, args.agency_id, {
                status: "completed",
                description,
    ...
    // Delete task
    server.tool("re_delete_task", `Delete a task.`, {
        id: z.string().describe("Task ID"),
        agency_id: AgencyIdSchema,
    }, async (args) => {
        try {
    ...
    server.tool("re_batch_update_tasks", `Update multiple tasks at once.
...
- "Complete all overdue follow-up tasks"`, {
        ...
        try {
            ...
            const updated = data;
            return createSuccessResponse(`# Updated ${updated.length} Task(s)!\n\n` +
                `**Changes**: ${[
