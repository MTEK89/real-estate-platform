 ...
 */
export function registerScheduleVisitTools(server) {
    server.tool("re_schedule_visit", `Schedule a property viewing with automatic contact and property resolution.
...
- "Set up a visit: new contact Pierre Martin, 621 123 456, property HSE-ABC, Friday 10am"`, {
        ...
        try {
            ...
            // Step 5: Create visit
            const { data: visit, error: visitError } = await insertRecord("visits", {
    ...
    // Quick reschedule
    server.tool("re_quick_reschedule", `Quickly reschedule a visit to a new date/time.
...
- "Reschedule to next Tuesday"`, {
        ...
        try {
            ...
            // Update visit
            const { data: updated, error: updateError } = await updateRecord("visits", args.visit_id, args.agency_id, {
                date: parsed.dateString,
            ...
            const property = existing.properties;
            const contact = existing.contacts;
...
//# sourceMappingURL=schedule-visit.js.map