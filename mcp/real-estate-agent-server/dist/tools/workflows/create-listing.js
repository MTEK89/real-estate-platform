 ...
 */
import { z } from "zod";
import { PropertyTypeSchema, AgencyIdSchema, AddressSchema, CharacteristicsSchema, } from "../../schemas/common.js";
 ...
 */
export function registerCreateListingTools(server) {
    server.tool("re_create_listing", `Create a new property listing from a natural language description.
...
- "New property: Office space, 500m², Kirchberg, owner is Jean Dupont"`, {
        ...
        try {
            ...
            if (args.owner_query) {
                ...
                if (ownerResult.contact) {
                    ...
                    const { data: newOwner, error: ownerError } = await insertRecord("contacts", {
            ...
            const { data: property, error: propError } = await insertRecord("properties", {
                reference,
                type: args.type,
            ...
            if (property.characteristics) {
                ...
                const charParts = [];
                if (chars.surface)
                    charParts.push(`${chars.surface} m²`);
                if (chars.bedrooms)
                    charParts.push(`${chars.bedrooms} bed`);
                if (chars.bathrooms)
                    charParts.push(`${chars.bathrooms} bath`);
    ...
    server.tool("re_quick_listing", `Create a minimal listing quickly - just the essentials.
...
- "Add a house listing for 600,000 in Strassen"`, {
        ...
        try {
            ...
            const { data: property, error } = await insertRecord("properties", {
                reference,
                type: args.type,
                status: "draft",
                price: args.price,
            ...
            return createSuccessResponse(`Quick listing created!\n\n` +
                `**${reference}** - ${args.type}\n` +
...
//# sourceMappingURL=create-listing.js.map