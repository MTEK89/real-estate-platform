...
import { formatDashboard, formatAddressShort } from "../../services/formatters.js";
...
export function registerDashboardTools(server) {
    server.tool("re_dashboard", `Get the agent's daily dashboard with today's schedule, urgent tasks, and pipeline summary.
...
- "What do I need to focus on?"`, {
        ...
        try {
            ...
            if (recentContactsResult.error)
                throw recentContactsResult.error;
            // Format visits
            ...
            const overdueTasks = allTasks.filter((t) => isPast(t.due_date) && !isToday(new Date(t.due_date)));
            ...
            const dashboardData = {
                ...
                recent_contacts: (recentContactsResult.data || []),
            };
    ...
    // Quick stats tool
    server.tool("re_quick_stats", `Get quick statistics about your real estate business.
...
- "Give me the numbers"`, {
        ...
        try {
            const supabase = getSupabaseClient();
            const [propertiesResult, contactsResult, visitsResult, tasksResult, contractsResult] = await Promise.all([
            ...
            const stats = {
                ...
                active_contracts: {
                    ...
                    mandates: contracts.filter((c) => c.type === "mandate").length,
                    sales: contracts.filter((c) => c.type === "sale").length,
                },
            };
            const format = args.format;
            if (format === "json") {
