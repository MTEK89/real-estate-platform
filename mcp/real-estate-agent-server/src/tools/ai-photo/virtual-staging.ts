...
import { VIRTUAL_STAGING_STYLES } from "../../constants.js";
import type { VirtualStagingStyle } from "../../constants.js";

/**
 * Register virtual staging tool
 */
export function registerVirtualStagingTools(server: McpServer): void {
  server.tool(
    "re_virtual_staging",
    `Add virtual furniture and décor to empty property photos.
...
- "Make this office look luxurious"`,
    ...
    async (args) => {
      try {
        ...
        const prompt = renderVirtualStagingPrompt({
          style: args.style as VirtualStagingStyle,
  ...
  server.tool(
    "re_quick_stage",
    `Quickly stage a room with automatic style detection.
