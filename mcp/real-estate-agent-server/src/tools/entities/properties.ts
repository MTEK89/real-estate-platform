...
- "3 bedroom houses in Luxembourg City" → type: "house", city: "Luxembourg", min_bedrooms: 3`,
    ...
    async (args) => {
      try {
        ...
        if (args.city) query = query.ilike("address->>city", `%${args.city}%`);
        if (args.min_bedrooms) query = query.gte("characteristics->>bedrooms", args.min_bedrooms);
        if (args.min_surface) query = query.gte("characteristics->>surface", args.min_surface);
        if (args.tags && args.tags.length > 0) query = query.overlaps("tags", args.tags);
        if (args.owner_id) query = query.eq("owner_id", args.owner_id);
  ...
  server.tool(
    "re_get_property",
    `Get a specific property by ID or reference number.
...
- "Update property APT-001 price to 500,000"`,
    ...
    async (args) => {
      try {
        ...
        if (args.id) {
          // Update existing
          const { data, error } = await updateRecord<Property>(
            "properties",
            args.id,
            args.agency_id,
            propertyData
          ...
          // Create new
          const { data, error } = await insertRecord<Property>("properties", propertyData);
        ...
        return createSuccessResponse(
          `# Property ${action.charAt(0).toUpperCase() + action.slice(1)}!\n\n` +
...
⚠️ This action cannot be undone. Consider archiving instead.`,
    ...
    async (args) => {
      try {
        const { data: property } = await getById<Property>("properties", args.id, args.agency_id);
  ...
  server.tool(
    "re_update_property_status",
    `Update a property's listing status.
...
- "Increase asking price to 500,000"`,
    ...
    async (args) => {
      try {
        ...
        return createSuccessResponse(
          `# Price ${change.charAt(0).toUpperCase() + change.slice(1)}!\n\n` +
...
- "Add these photos to property APT-001"`,
    ...
    async (args) => {
      try {
        ...
        const { error } = await updateRecord<Property>(
          "properties",
          result.property.id,
          args.agency_id,
          { images: newImages }
