...
import { formatContract, formatContractList, formatPrice } from "../../services/formatters.js";
import { createErrorResponse, createSuccessResponse } from "../../utils/errors.js";
import { parseDate, formatDateDisplay } from "../../services/date-parser.js";
import type { Contract, Property, Contact } from "../../types.js";
...
- "Pending sales contracts" → type: "sale", status: "pending"`,
    ...
    async (args) => {
      try {
        ...
        const contracts = (data || []).map((row: Record<string, unknown>) => ({
  ...
  server.tool(
    "re_create_contract",
    `Create a new contract (mandate, sale, or rental).
...
- "New mandate for Marie's property, 3% commission, exclusive"`,
    ...
    async (args) => {
      try {
        ...
        const { data, error } = await insertRecord<Contract>("contracts", contractData);
...
- "Complete the sale contract" → status: "completed"`,
    ...
    async (args) => {
      try {
        const { data: existing, error: fetchError } = await getById<Contract>("contracts", args.id, args.agency_id);
        ...
        const { data, error } = await updateRecord<Contract>("contracts", args.id, args.agency_id, updateData);
  ...
  server.tool(
    "re_sign_contract",
    `Mark a contract as signed with optional signing date.`,
    {
      id: z.string().describe("Contract ID"),
    ...
    async (args) => {
      try {
        ...
        const { data, error } = await updateRecord<Contract>("contracts", args.id, args.agency_id, {
          status: "signed",
...
⚠️ This action cannot be undone. Consider changing status to cancelled instead.`,
    ...
    async (args) => {
      try {
        ...
        return createSuccessResponse(
          `Contract deleted: **${existing.type}** contract (\`${existing.id}\`)`
        );
      } catch (error) {
        return createErrorResponse(error);
      }
