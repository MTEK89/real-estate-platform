...
import { parseDateAndTime, formatDateString, formatDisplayDate } from "../../services/date-parser.js";
import type { Contract, Contact, Property, Task } from "../../types.js";

/**
 * Register prepare contract workflow tool
 */
export function registerPrepareContractTools(server: McpServer): void {
  server.tool(
    "re_prepare_contract",
    `Prepare a contract for a property and contact.
...
- "Draft a rental agreement for the apartment with Pierre, 2000/month"`,
    ...
    async (args) => {
      try {
        ...
        const { data: contract, error: contractError } = await insertRecord<Contract>("contracts", contractData);
        ...
        if (args.type === "mandate") {
          next_steps.push(
            "Review contract terms with property owner",
            "Discuss exclusivity and commission",
            "Get contract signed",
            "Start marketing the property"
          );
        } else if (args.type === "sale_existing" || args.type === "sale_vefa") {
          next_steps.push(
            "Review terms with buyer and seller",
          ...
          next_steps.push(
            ...
            "Plan move-in inspection"
          );
        } else if (args.type === "offer") {
          next_steps.push(
            "Review offer terms with buyer",
            "Present offer to seller",
            "Negotiate if needed",
            "Prepare sale contract if accepted"
          );
  ...
  server.tool(
    "re_quick_mandate",
    `Quickly create a selling mandate for a property.
...
- "Quick mandate: HSE-XYZ, Marie Dupont, 3% commission, exclusive"`,
    ...
    async (args) => {
      try {
        ...
        // Create mandate
        const { data: contract, error } = await insertRecord<Contract>("contracts", {
          type: "mandate",
