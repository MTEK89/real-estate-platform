...
import type { Contact, Property, Visit } from "../../types.js";

/**
 * Register draft email workflow tools
 */
export function registerDraftEmailTools(server: McpServer): void {
  server.tool(
    "re_draft_email",
    `Draft a professional email for real estate communications.
...
- "Send a follow-up email after the visit with Jean"`,
    ...
    async (args) => {
      try {
        ...
        let visit: (Visit & { property?: Property }) | null = null;
        if (args.visit_id) {
  ...
  server.tool(
    "re_email_property",
    `Quick shortcut to draft a property presentation email.
...
- "Send Marie the details of HSE-XYZ"`,
  ...
  server.tool(
    "re_email_visit_confirmation",
    `Draft a visit confirmation email.
...
- "Send confirmation for visit [id]"`,
    ...
    async (args) => {
      try {
        ...
        const property = (data as any).properties as Property;
        const contact = (data as any).contacts as Contact;
...
interface EmailGeneratorOptions {
  ...
  visit: (Visit & { property?: Property }) | null;
  tone?: string;
...
function generateEmail(options: EmailGeneratorOptions): { subject: string; body: string } {
...
function getGreeting(contact: Contact, tone: string, language: string): string {
...
function getClosing(tone: string, language: string): string {
...
function generatePropertyPresentationEmail(
  ...
  if (includeCharacteristics && property.characteristics) {
    ...
    const features = [];
    if (chars.surface) features.push(`${chars.surface} m²`);
    if (chars.bedrooms) features.push(`${chars.bedrooms} bedrooms`);
    if (chars.bathrooms) features.push(`${chars.bathrooms} bathrooms`);
...
function generateVisitConfirmationEmail(
  options: EmailGeneratorOptions,
  greeting: string,
  closing: string
): { subject: string; body: string } {
...
function generateVisitFollowupEmail(
  options: EmailGeneratorOptions,
  greeting: string,
  closing: string
): { subject: string; body: string } {
...
function generateOfferReceivedEmail(
  options: EmailGeneratorOptions,
  greeting: string,
  closing: string
): { subject: string; body: string } {
...
function generateContractReadyEmail(
  options: EmailGeneratorOptions,
  greeting: string,
  closing: string
): { subject: string; body: string } {
